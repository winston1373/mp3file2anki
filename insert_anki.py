...
def create_anki_cards(deck_name, audio_dir, whisper_dir, filename):
    ...
    audio_subtitle_dir = os.path.join(whisper_dir, filename)

    for txt_file in sorted(os.listdir(audio_subtitle_dir)):
        ...
        mp3_dir = os.path.join(audio_dir, filename)
        mp3_path = os.path.join(mp3_dir, mp3_name_pitch)
        mp3_path_slow = os.path.join(mp3_dir, mp3_name_slow)
        ...
        # Read subtitle text
        txt_path = os.path.join(audio_subtitle_dir, txt_file)
...
def main():
    ...
    mp3file = sys.argv[1]
    create_anki_cards("Japanese Listening", "output_segments", "whisper_output/chunk", mp3file)
